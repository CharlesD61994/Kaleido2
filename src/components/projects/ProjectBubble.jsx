...
<div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8, padding: isLibrary ? "12px 4px 14px" : "8px 4px 8px", cursor: "default" }}>
  <div style={{ position: "relative", width: size, height: size, overflow: "visible", isolation: "isolate" }}>
    ...
    {onMenuOpen && <button onClick={(e) => { e.stopPropagation(); onMenuOpen(project, e); }}
      style={{ position: "absolute", top: isLibrary ? -9 : -9, right: isLibrary ? -9 : -9, transform: "translate(12%, -20%)", width: 24, height: 24, borderRadius: "50%", background: isCompleted ? "linear-gradient(135deg, #403B4D, #242231)" : `linear-gradient(135deg, ${color.light}, ${color.bg})`, border: "2.5px solid var(--k-bg)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontStyle: "italic", fontWeight: 700, color: isCompleted ? "#9B95AA" : "#fff", boxShadow: isCompleted ? "0 4px 10px rgba(0,0,0,0.28)" : `0 0 0 3px ${color.light}24, 0 6px 14px rgba(0,0,0,0.35)`, animation: isCompleted ? "none" : "infoBob 2.6s ease-in-out infinite", transition: "transform 220ms cubic-bezier(0.22, 1, 0.36, 1), box-shadow 220ms cubic-bezier(0.22, 1, 0.36, 1), filter 220ms ease", zIndex: 10 }}
