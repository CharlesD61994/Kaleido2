...
export default function EditPdfPatronModal({ asPage = false, patron, onClose, onSave }) {
  const initialParties = (patron.pdfParties || []).map((partie, index) => ({
    id: partie.id,
    nom: partie.nom,
  ...
  const locateRepeatPartie = (globalRang) => {
    let offset = 0;
    for (const partie of initialParties) {
      const total = parseInt(partie.rangs, 10) || 0;
      if (globalRang <= offset + total) return { partieId: partie.id, localRang: Math.max(1, globalRang - offset) };
      offset += total;
    }
    return { partieId: initialParties[0]?.id || "", localRang: Math.max(1, globalRang || 1) };
  };
  ...
  const [repetitions, setRepetitions] = useState((patron.pdfRepetitions || []).map((repeat) => {
    if (repeat.partieId) return repeat;
    const start = locateRepeatPartie(parseInt(repeat.startRang, 10) || 1);
